...
// Обычный архетип живёт внутри safe zone: всё, что он рисует, помещается в сцену
// и не уезжает под обрезку платформы. Мультяшный кадр намеренно другой —
// декорация занимает кадр целиком, как в анимации, а брендовая полоса и точки
// прогресса ложатся поверх неё. Список нужен, чтобы «рисую больше сцены»
// оставалось объявленным решением, а не случайностью: у full-bleed архетипа
// декорация выходит за safe zone, но текст и облачко реплики — нет.
export const FULL_BLEED_ARCHETYPES = Object.freeze(["cartoon-shot"]);
