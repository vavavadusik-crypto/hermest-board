...
ACTIVE PHASE: между фазами. Закрыты 1–8 + 9 (CI offload) + MLE (мультиязычные издания @ 9583bc4, CI SUCCESS — весь heavy-гейт с реальным ffmpeg зелёный). NEXT (по слову Вадима): ещё функционал ИЛИ PHASE 10 (документация) + PHASE 11 (release candidate). MLE-follow-ups (не блокеры): RTL/CJK шрифты, bulk все-языки, ручная правка перевода, prefix-strip hardening.
